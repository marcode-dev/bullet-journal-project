...
function processar(dadosImportados) {
    ...
    const ids = new Set()
    dadosImportados = dadosImportados.filter(item => {
        if (ids.has(item.idElemento)) {
            return
        }
        ids.add(item.idElemento);
        return true;
    })
    ...
    if (tipoImportacao.value === "adicionar") {
        ...
        abrirModal(
            ...
            `<p>${dadosImportados.length} tarefas importadas. ${nSubstituido.length} tarefas substituídas.</p>`, 
            null, 
            "success"
        );
        ...
        const callback = () => {
            abrirModal(
                "Importação Concluída",
                `<p>${dadosImportados.length} tarefas importadas, substituindo todas as tarefas anteriores.</p>`,
                null,
                "success"
            );
        ...
        abrirModal(
            ...
            `<p>Tem certeza que deseja substituir todas as tarefas atuais por estas ${dadosImportados.length} tarefas importadas?</p>`,
            callback,
            "confirmar"
        );
    }
